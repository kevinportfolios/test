...
try {
    ...
    foreach ($getKline["result"]["list"] as $r) {
        $closePrice20[]=$r["4"];
        // $closePrice20[]=$r["4"]+45;//1min
        // $closePrice20[]=$r["4"]+56;//5min
    }
    $closePrice4 = array_slice($closePrice20, 0, -16);
...
if($is_running_order==0){
    ...
    if($finalema10>$finalema20){
        ...
        if($position =="long to short"){
            ...
            if ($allBigger) {
                logger('update position status long:'.json_encode($closePrice4));
                $position ="short to long";
            }
        ...
        if($position =="short to long"){
            ...
            if ($allBigger) {
                logger('update position status short:'.json_encode($closePrice4));
                $position ="long to short";
            }
        ...
        //close 做多
            ...
            if(($diffprice>300)){
                if($before1minprice>($currentprice)){
                    logger('diffprice>300 long:'.date('Y-m-d H:i:s',$time).';buyingprice:'.$buyingprice.';currentprice:'.$currentprice);
       ...
       //close 做空
            ...
            if(($diffprice>300)){
                if(($currentprice)>$before1minprice){
                    logger('diffprice>300 short:'.date('Y-m-d H:i:s',$time).';buyingprice:'.$buyingprice.';currentprice:'. $currentprice);
