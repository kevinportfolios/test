...
try {
    ...
    if($isfirstorder==1&&($closePrice20[0]>$finalema10)&&($closePrice20[0]>$finalema20)){
        //做空
        orderloggertestvol('position111:'.$position.'time:'.date('Y-m-d H:i:s',($endtime/1000+(8*3600))));
        if($position!="long to short"){
            $exceedprice = $currentprice;
            $isfirstorder=0;
            $position ="short to long";
            loggertestvol("start order1");
            loggertestvol('update exceedprice111:'.date('Y-m-d H:i:s',($endtime/1000+(8*3600))).';exceedprice:'.$exceedprice);
        }
        
    }else if($isfirstorder==1&&($finalema10>$closePrice20[0])&&($finalema20>$closePrice20[0])){
        //做多
        orderloggertestvol('position222:'.$position.'time:'.date('Y-m-d H:i:s',($endtime/1000+(8*3600))));
        if($position!="short to long"){
            $exceedprice = $currentprice;
            $isfirstorder=0;
            $position ="long to short";
            loggertestvol("start order2");
            loggertestvol('update exceedprice222:'.date('Y-m-d H:i:s',($endtime/1000+(8*3600))).';exceedprice:'.$exceedprice);
        }
    }

    //
    if($isfirstorder==0&&$position=="short to long"&&($finalema10>($closePrice20[0]))&&($finalema20>($closePrice20[0]))){
        orderloggertestvol('position333:'.$position.'time:'.date('Y-m-d H:i:s',($endtime/1000+(8*3600))));
        $exceedprice = $currentprice;
        loggertestvol('update exceedprice333:'.date('Y-m-d H:i:s',($endtime/1000+(8*3600))).';exceedprice:'.$exceedprice.';finalema10:'.$finalema10.';finalema20:'.$finalema20);
        $position ="long to short";
    }else if($isfirstorder==0&&$position=="long to short"&&($closePrice20[0]>($finalema10))&&($closePrice20[0]>($finalema20))){
        orderloggertestvol('position444:'.$position.'time:'.date('Y-m-d H:i:s',($endtime/1000+(8*3600))));
        $exceedprice = $currentprice;
        loggertestvol('update exceedprice444:'.date('Y-m-d H:i:s',($endtime/1000+(8*3600))).';exceedprice:'.$exceedprice.';finalema10:'.$finalema10.';finalema20:'.$finalema20);
        $position ="short to long";
    }
